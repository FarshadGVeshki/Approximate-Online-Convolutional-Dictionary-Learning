function [X, res] = CSC(D,  S, lamb, opts)
...
vec = @(x) x(:);
%%% rotated normalized dictionary filters
tsrt = tic;
Df = fft2(D,H,W);
SDD = sum(abs(Df).^2,3);
Cf = conj(Df)./(SDD+rho);
...
while itr <= MaxIter && (r > epri || s > edua)
    ...
    [Zf,Cf,rho_flg]  = X_update(fft2(X-U),Cf,Df,Sf,SDD,rho,rho_flg) ;
    ...
    res.iterinf = [res.iterinf; [itr fval rPow JL1 r s titer]];
    res.U = U;
...
function [Zf,Cf,rho_flg]  = X_update(Wf,Cf,Df,Sf,SDD,rho,rho_flg)
